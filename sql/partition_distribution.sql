-- Test the correctness of distribution of partitions among segments
...
-- Test: One partition per segment

-- start_ignore
SELECT gp_inject_fault('all', 'reset', dbid)
FROM gp_segment_configuration;

CREATE FOREIGN TABLE test_kadb_fdw_t1(i INT, t TEXT)
...
SELECT i, t FROM test_kadb_fdw_t1;
...
SELECT prt, off
FROM kadb.offsets
WHERE ftoid = 'test_kadb_fdw_t1'::regclass::oid
ORDER BY prt;


-- Test: Half partition per segment (1 out of 3 segments gets partitions)
...
-- Test: All segments without partitions
...
-- Test: Two partitions per segment
